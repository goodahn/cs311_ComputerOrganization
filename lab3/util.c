...
/***************************************************************/
/* stage register and register                                 */
/***************************************************************/

struct if_id_latch_struct if_register = {};
struct if_id_latch_struct if_id_latch = {};
struct id_ex_latch_struct id_register = {};
struct id_ex_latch_struct id_ex_latch = {};
struct ex_mem_latch_struct ex_register = {};
struct ex_mem_latch_struct ex_mem_latch = {};
struct mem_wb_latch_struct mem_register = {};
struct mem_wb_latch_struct mem_wb_latch = {};
struct mem_wb_latch_struct wb_register = {};

int branch_prediction;
int branch_target_buffer[4] = {0,};
int data_forwarding;
int pipeline_dump_set;
int CYCLE;
...
uint32_t mem_read_32(uint32_t address) {
	...
	printf("Memory Read Error: Exceed memory boundary %p\n",address);
	exit(-1);
...
void mem_write_32(uint32_t address, uint32_t value) {
	...
	if( address ==0 ){
		printf("Memory Write Error: Exceed memory boundary 0x0\n");
		exit(-1);
	} else{
		printf("Memory Write Error: Exceed memory boundary %p\n",address);
		exit(-1);
	}
}
...
/* Procedure : pdump                                           */
/*                                                             */
/* Purpose   : print instruction of five pipeline stage        */
/*                                                             */
/***************************************************************/
void pdump() {
    int k;
    ...
    if( if_register.PC !=0  ) {
      printf("0x%08x", if_register.PC);
    } else {
      printf("          ");
    }
    printf("|");
    if( id_register.PC != 0 ) {
      printf("0x%08x", id_register.PC);
    } else {
      printf("          ");
    }
    printf("|");
    if( ex_register.PC != 0 ) {
      printf("0x%08x", ex_register.PC);
    } else {
      printf("          ");
    }
    printf("|");
    if( mem_register.PC != 0 ) {
      printf("0x%08x", mem_register.PC);
    } else {
      printf("          ");
    }
    printf("|");
    if( wb_register.PC != 0 ) {
      printf("0x%08x", wb_register.PC);
    } else {
      printf("          ");
    }
    printf("\n\n");
...
void btb_move_right() {
  branch_target_buffer[3] = branch_target_buffer[2];
  branch_target_buffer[2] = branch_target_buffer[1];
  branch_target_buffer[1] = branch_target_buffer[0];
}
